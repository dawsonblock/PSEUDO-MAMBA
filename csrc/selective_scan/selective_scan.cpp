...
// State-aware variant that returns final SSM state
template <typename input_t, typename weight_t>
void selective_scan_fwd_with_state_cuda(
    SSMParamsBase &params,
    void *x_final_ptr, // Output: final state [batch, dim, 2*dstate]
    cudaStream_t stream);
...
void set_ssm_params_bwd(
    SSMParamsBwd &params,
    // sizes
    const size_t batch, const size_t dim, const size_t seqlen,
    const size_t dstate, const size_t n_groups, const size_t n_chunks,
    const bool is_variable_B, const bool is_variable_C,
    // device pointers
    const at::Tensor u, const at::Tensor delta, const at::Tensor A,
    const at::Tensor B, const at::Tensor C, const at::Tensor z,
    const at::Tensor out, const at::Tensor out_z, void *D_ptr,
    void *delta_bias_ptr, void *x_ptr, const at::Tensor dout,
    const at::Tensor du, const at::Tensor ddelta, const at::Tensor dA,
    const at::Tensor dB, const at::Tensor dC, const at::Tensor dz, void *dD_ptr,
    void *ddelta_bias_ptr, bool has_z, bool delta_softplus,
    bool recompute_out_z) {
  // Pass in "dout" instead of "out", we're not gonna use "out" unless we have z
  set_ssm_params_fwd(params, batch, dim, seqlen, dstate, n_groups, n_chunks,
                     is_variable_B, is_variable_C, u, delta, A, B, C,
                     has_z ? out : dout, has_z ? z : dout,
                     // If not recompute_out_z, pass dout instead of out_z.
                     // This won't be used by the bwd kernel
                     recompute_out_z ? out_z : dout, D_ptr, delta_bias_ptr,
                     x_ptr, has_z, delta_softplus);
  if (!recompute_out_z) {
...
std::vector<at::Tensor> selective_scan_bwd(
    const at::Tensor &u, const at::Tensor &delta, const at::Tensor &A,
    const at::Tensor &B, const at::Tensor &C,
    const c10::optional<at::Tensor> &D_, const c10::optional<at::Tensor> &z_,
    const c10::optional<at::Tensor> &delta_bias_, const at::Tensor &dout,
    const c10::optional<at::Tensor> &x_, const c10::optional<at::Tensor> &out_,
    c10::optional<at::Tensor> &dz_, bool delta_softplus, bool recompute_out_z) {
  auto input_type = u.scalar_type();
  auto weight_type = A.scalar_type();
  TORCH_CHECK(input_type == at::ScalarType::Float ||
              input_type == at::ScalarType::Half ||
  ...
  at::Tensor dB =
      !is_variable_B ? torch::zeros_like(B)
                     : torch::zeros_like(B, B.options().dtype(torch::kFloat32));
  at::Tensor dC =
      !is_variable_C ? torch::zeros_like(C)
  ...
  set_ssm_params_bwd(
      params, batch_size, dim, seqlen, dstate, n_groups, n_chunks,
      is_variable_B, is_variable_C, u, delta, A, B, C, z, out, out_z,
      D_.has_value() ? D_.value().data_ptr() : nullptr,
      delta_bias_.has_value() ? delta_bias_.value().data_ptr() : nullptr,
      x_.has_value() ? x_.value().data_ptr() : nullptr, dout, du, ddelta, dA,
      dB, dC, dz, D_.has_value() ? dD.data_ptr() : nullptr,
      delta_bias_.has_value() ? ddelta_bias.data_ptr() : nullptr, has_z,
  ...
  DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(
      u.scalar_type(), "selective_scan_bwd", [&] {
        DISPATCH_WTYPE_FLOAT_AND_COMPLEX(
            A.scalar_type(), "selective_scan_bwd", [&] {
              selective_scan_bwd_cuda<input_t, weight_t>(params, stream);
            });
      });
  std::vector<at::Tensor> result = {
      du, ddelta, dA, dB.to(B.dtype()), dC.to(C.dtype()), dD, ddelta_bias};
  if (has_z) {
    result.push_back(dz);
  }
  if (recompute_out_z) {
...
std::tuple<at::Tensor, at::Tensor> selective_scan_fwd_with_state(
    const at::Tensor &u, const at::Tensor &delta, const at::Tensor &A,
    const at::Tensor &B, const at::Tensor &C,
    const c10::optional<at::Tensor> &D_, const c10::optional<at::Tensor> &z_,
    const c10::optional<at::Tensor> &delta_bias_,
    const c10::optional<at::Tensor> &x0_, // Initial state
    bool delta_softplus, bool return_last_state) {
  auto input_type = u.scalar_type();
  auto weight_type = A.scalar_type();
  TORCH_CHECK(input_type == at::ScalarType::Float ||
              input_type == at::ScalarType::Half ||
  ...
  // Allocate final state output
  at::Tensor x_final = torch::empty({batch_size, dim, dstate * 2},
                                    u.options().dtype(weight_type));
  ...
  DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(
      u.scalar_type(), "selective_scan_fwd_with_state", [&] {
        DISPATCH_WTYPE_FLOAT_AND_COMPLEX(
            A.scalar_type(), "selective_scan_fwd_with_state", [&] {
              // For now, use the standard forward pass
              // In a full implementation, you'd call
              // selective_scan_fwd_with_state_cuda which extracts and writes
              // final state
              selective_scan_fwd_cuda<input_t, weight_t>(params, stream);

              // Extract final state from x (last chunk)
              // x shape: [batch, dim, n_chunks, 2*dstate]
              // We want the last chunk: x[:, :, -1, :]
              if (return_last_state) {
                x_final.copy_(x.select(2, n_chunks - 1)); // Select last chunk
              } else {
                x_final.zero_();
              }
            });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("fwd_with_state", &selective_scan_fwd_with_state,
        "Selective scan forward with explicit final state output");
}
