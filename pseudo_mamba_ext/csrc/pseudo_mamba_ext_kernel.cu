...
template <typename scalar_t>
__global__ void
pseudo_mamba_backward_kernel(const scalar_t *__restrict__ grad_out,
                             const scalar_t *__restrict__ y,
                             scalar_t *__restrict__ grad_x,
                             scalar_t *__restrict__ grad_h, const int64_t N) {
  const int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < N) {
    const scalar_t dy = static_cast<scalar_t>(1) - y[idx] * y[idx];
