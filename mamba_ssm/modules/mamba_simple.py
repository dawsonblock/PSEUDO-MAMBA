...
class MambaInferenceState:
    """Encapsulates the internal recurrent state of a Mamba layer during inference.
    
    Args:
        conv_state: Convolution sliding window state [B, D, d_conv-1]
    ...
    def to(self, device=None, dtype=None):
        ...
        return MambaInferenceState(
            conv_state=self.conv_state.to(
                device=device,
                dtype=dtype or self.conv_state.dtype
            ),
            ssm_state=self.ssm_state.to(
                device=device,
                dtype=dtype or self.ssm_state.dtype
            ),
        )
